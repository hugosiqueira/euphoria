...
class Auth extends Model
{
     ...
     * @return User|null
     */
    public function attempt(string $email, string $password, int $roles = 1): ?User
    {
        if (!is_email($email)) {
     ...
     * @return bool
     */
    public function login(string $email, string $password, bool $save = false, int $roles = 1): bool
    {
        $user = $this->attempt($email, $password, $roles);
        if (!$user) {
            return false;
        }
    ...
    public function forget(string $email): bool
    {
        ...
        $message = $view->render("forget", [
            ...
            "forget_link" => url("admin/forget/{$user->email}|{$user->forget}")
        ]);
